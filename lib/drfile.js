...
module.exports = function(source,dest,options,callback){
    ...
    fse.ensureDirSync(basePath(source), function (err) {
        if(err){
            console.log(err)
            cback(err);
        }
    ...
    if(options.deldiff){
        fse.readFile(source, 'utf8', (err, data)=> {
            if(err){
                cback(err);
            }
            var files = [];
    ...
    function pack(){
        ...
        const destDir =(behave =='-bak-'?basePath(bakdir+source+behave+getVersion()+'.zip'):basePath(source+behave)+'.zip') ;
